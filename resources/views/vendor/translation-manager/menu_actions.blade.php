<form class="form-import pull-left" method="POST" action="{{ route('translation-manager.import') }}" role="form" style="margin-right: 20px">
	@if(sizeof($groups) == 0)
	<a href="#">
		<button type="submit" class="btn btn-primary" title="{{ trans('panel.actions.import') }}"><i class="glyphicon glyphicon-download"></i></button>
	</a>
	@else
	<div class="btn-group">
		<button type="button" class="btn btn-primary dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="{{ trans('panel.actions.import') }}">
...
<form class="form-export pull-left" method="POST" action="{{ route('translation-manager.publish', '*') }}" role="form" style="margin-right: 20px">
	{{ csrf_field() }}
	<button type="submit" class="btn btn-primary" title="{{ trans('panel.actions.export') }}"><i class="glyphicon glyphicon-upload"></i></button>
</form>
...
<form class="form-search pull-left" method="POST" action="{{ route('translation-manager.find') }}" role="form">
	{{ csrf_field() }}
	<button type="submit" class="btn btn-primary" title="{{ trans('panel.actions.find') }}"><i class="glyphicon glyphicon-search"></i></button>
</form>
